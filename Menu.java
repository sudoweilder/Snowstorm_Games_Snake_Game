...
public class Menu {
    ...
    public static void mainMenu(Scanner console) throws FileNotFoundException, InterruptedException { // Main menu with different options and leads to everything(so far)
        ...
        if (choice == 1) { // runs the main game loop
            ...
            Game.gameloop(null); // runs the snake game
            ...
            else System.out.println("Invaild Entry"); mainMenu(console);
        }
    ...
    public static void boardSize(Scanner console) {
        System.out.print("Set board size:\n"
                        ...
                        + "(Type 1, 2, or 3): ");
        int choice = console.nextInt();
        if (choice == 1) Game.panelWidth = 300;
        else if (choice == 2) Game.panelWidth = 500;
        else if (choice == 3) Game.panelWidth = 700; // max size; panel doesn't fit screen well when over 700
        else {
            System.out.println("Invaild"); 
            boardSize(console);
        }
    }
    public static void credits(Scanner console) throws FileNotFoundException, InterruptedException {
        System.out.print("|---------------Credits-----------------|\n"
                        + " Made by: Connor, Jackson, Miles, Robert\n\n"
                        + " Press 1 to return to the main menu\n"
                        + "|---------------------------------------|\n"
                        + "Type: ");
        if (console.nextInt() == 1) mainMenu(console);
        else System.out.println("Invaild Entry"); credits(console);
    }  
